...
$videoEmbed = $content['field_soundcloud_embed'][0]['#markup'];

//echo $videoEmbed;

//echo '<pre style="display: none">';print_r(json_decode($videoEmbed));echo '</pre>';
...
<?php /*<article id="node-<?php print $node->nid; ?>" class="<?php print $classes; ?> clearfix"<?php print $attributes; ?>> */ ?>

<?php
/*echo $title;
echo $content[0]['#markup'].'<br>';
if(render($content['field_thumb_image'])){ echo render($content['field_thumb_image']).'<br>';}
if(render($content['field_cover_image'])){echo render($content['field_cover_image']).'<br>';}
//echo render($content['field_brightcove_video']).'<br>';
echo render($content['field_menu_category']).'<br>';
echo render($content['field_article_category']).'<br>';

print render($content['field_soundcloud_embed']);*/
?>
	...
	<div class="titleBar" style="display:none;">
		<div class="titleBarWrap">
			<div class="titleBarLhs">
				 ...
				 <div class="postMeta">
					<span class="date"><?php echo date('jS M, Y', $node->created);?></span> | <span class="tag"> <?php echo $content['field_article_category'][0]['#markup']; ?> </span>
				 </div>
			</div> 
	...
	<div class="postDetail">
		<div class="videoDetail">
            <div class="videoOverlay">
                <img class="imgOverlay" src="<?php echo $pageImage; ?>" alt="<?php echo $imageAlt; ?>" title="<?php echo $imageTitle; ?>" />
            </div>
		</div>
		...
		<div class="postDetailContent">
			<div class="videoDesc">
				...
				<div class="summary"><?php print $body[0]['value']; ?></div>
                
                <?php print_r($videoEmbed); ?>
		...
		<div class="wetPaintContent">
			<?php foreach($similarStories->result as $sKey=>$similarStories){
				...
				if($similarNode->type == 'blogs' || $similarNode->type == 'listicles'){
					$similarStoryContentTypeImage = '/' . drupal_get_path('theme', 'adaptivetheme_subtheme') . '/images/listicle-icon.png';
				}else if($similarNode->type == 'photo_essay'){
					$similarStoryContentTypeImage = '/' . drupal_get_path('theme', 'adaptivetheme_subtheme') . '/images/pic-icon.png';
				}else if($similarNode->type == 'prodcast'){
					$similarStoryContentTypeImage = '/' . drupal_get_path('theme', 'adaptivetheme_subtheme') . '/images/podcast-icon.png';
				}else if($similarNode->type == 'series'){
					$similarStoryContentTypeImage = '/' . drupal_get_path('theme', 'adaptivetheme_subtheme') . '/images/series-icon.png';
				}else if($similarNode->type == 'videos'){
					$similarStoryContentTypeImage = '/' . drupal_get_path('theme', 'adaptivetheme_subtheme') . '/images/video-icon.png';
				}
				...
				<div class="description">
					...
					<div class="mediaIconHolder">
						<div class="mediaIcon video">
							<img src="<?php echo $similarStoryContentTypeImage; ?>" />
						</div>
						<div class="mediaIcon more">
							<div class="socialShare">
				                ...
				                <ul class="postButton">
									...
									<?php if($similarNode->type != 'series'){ ?>
