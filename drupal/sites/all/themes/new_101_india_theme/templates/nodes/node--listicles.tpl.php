...
/*foreach($mostPopResArrSorted as $mosKey=>$mos){
    if($mos->node_type == 'videos'){
        echo '<pre style="display:none">';print_r($mos->field_field_brightcove_video[0]['rendered']['#video']->length);echo '</pre>';
        //echo '<pre style="display:none">';print_r(render($mos->field_field_brightcove_video));echo '</pre>';
        break;
    }
}*/
...
drupal_add_js(drupal_get_path('theme', 'new_101_india_theme') . '/scripts/node-details-sticky.js',array('type' => 'file', 'group' => JS_THEME, 'weight' => 100));
?>
...
<div class="pageBanner article">
    ...
    <div class="articleContent">
        <div class="lhsContent">
            ...
            <div class="articleSummary">
                ...
                <?php if(isset($body[0]['value']) && !empty($body[0]['value'])){
                    echo $body[0]['value'];
                } ?>
            </div>
            
            <?php $title = htmlentities($title); ?>
            
            <div class="articleSummary">
                <ul>
                <?php 
                $listicles = $content['field_listicles']['#items'];
                foreach($listicles as $key=>$val){
                ?>
                    <li>
                        <?php if(isset($val['field_listicle_title']['und']) && !empty($val['field_listicle_title']['und'])){ ?>
                        <h3>
                            <?php echo ($key+1).'. '.$val['field_listicle_title']['und'][0]['value']; ?>
                        </h3>
                        <?php } ?>

                        <?php if(isset($val['field_listicle_body']['und']) && !empty($val['field_listicle_body']['und'])){
                            echo $val['field_listicle_body']['und'][0]['value'];
                        } ?>

                        <?php if(isset($val['field_image_credit']['und']) && !empty($val['field_image_credit']['und'])){?>
                            <div class="source"><?php echo $val['field_image_credit']['und'][0]['value']; ?></div>
                        <?php } ?>

                        <?php if(isset($val['field_soundcloud_embed']['und']) && !empty($val['field_soundcloud_embed']['und'])){
                            $sounclodURL = urlencode($val['field_soundcloud_embed']['und'][0]['url']);
                            $soundcloudCurlUrl = 'http://soundcloud.com/oembed?iframe=true&format=json&url='.$sounclodURL.'';
                            $soundcloutData = soundcloudfield_curl_get($soundcloudCurlUrl);
                            //echo '<pre style="display: none">';print_r(json_decode($soundcloutData, 1));echo '</pre>';
                            $souncloudIframe = json_decode($soundcloutData, 1);
                            $souncloudIframe = $souncloudIframe['html'];
                            $souncloudIframe = preg_replace('/(height=)"([^"]+)"/', 'height="166"', $souncloudIframe);
                            if (preg_match('/auto_play=(true|false)/', $souncloudIframe)) {
                                $souncloudIframe = preg_replace('/auto_play=(true|false)/', 'auto_play=false', $souncloudIframe);
                            }
                            else {
                                $souncloudIframe = preg_replace('/">/', '&auto_play=false">', $souncloudIframe);
                            }
                              // Show playcount?
                            if (preg_match('/show_playcount=(true|false)/', $souncloudIframe)) {
                                $souncloudIframe = preg_replace('/show_playcount=(true|false)/', 'show_playcount=false', $souncloudIframe);
                            }
                            else {
                                $souncloudIframe = preg_replace('/">/', '&show_playcount=flase">', $souncloudIframe);
                            }
                            // Set player color.
                            if (preg_match('/color=([a-zA-Z0-9]{6})/', $final_iframe)) {
                                $souncloudIframe = preg_replace('/color=([a-zA-Z0-9]{6})/', 'color=1591fd', $souncloudIframe);
                            }
                            else {
                                $souncloudIframe = preg_replace('/">/', '&color=1591fd">', $souncloudIframe);
                            }
                            if (preg_match('/visual=(true|false)/', $souncloudIframe)) {
                                $souncloudIframe = preg_replace('/visual=(true|false)/', 'visual=false' , $souncloudIframe);
                            }else {
                                $souncloudIframe = preg_replace('/">/', '&visual=false">', $souncloudIframe);
                            }
                            if (preg_match('/show_artwork=(true|false)/', $souncloudIframe)) {
                                $souncloudIframe = preg_replace('/show_artwork=(true|false)/', 'show_artwork=true' , $souncloudIframe);
                            }else {
                                $souncloudIframe = preg_replace('/">/', '&show_artwork=true">', $souncloudIframe);
                            }
                            print $souncloudIframe;
                        } ?>
                    </li>
                <?php } ?>
                </ul>
        ...
        <div class="rhsContent">
            <div class="socialContent" id="rhsHeader">
                ...
                <div class="popularContent">
                    ...
                    <?php foreach($mostPopResArrSorted as $kMos=>$mosPop){
                        if($mostPopCounter < 8){
                            ...
                            else if(!empty($mosPop->_field_data['nid']['entity']->field_vzaar_video_url['und'][0]['value'])){
                                $parts = explode("/", $mosPop->_field_data['nid']['entity']->field_vzaar_video_url['und'][0]['value']);
    ...
    <div class="relatedVideosSection">
        ...
        <?php foreach($similarStoriesRes as $kSim=>$similarStories){
            if($similarCounter < 5){
                ...
                $similarTitle = htmlentities($similarNode->title);
    ...
    <div class="relatedVideosSection">
        ...
        <?php foreach($srView->result as $kSer=>$seriesStories){
            ...
            $seriesTitle = htmlentities($seriesNode->title);
