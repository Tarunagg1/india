...
$similarArticleTypeName = $fields['field_article_category']->content;
if(strstr($similarArticleTypeName,'101')){
	if($similarArticleTypeName == "101 Travel"){
		$typeLink = $base_url.'/travel-food';
	}else if($similarArticleTypeName == "101 Janta"){
		$typeLink = $base_url.'/people';
	}else{
		$typeLink = $base_url.'/'.strtolower(str_replace('101 ','',$similarArticleTypeName));
	}
}else{
	$typeLink = $base_url.'/'.strtolower($similarArticleTypeName);
}
?>
